...
import { MdFastfood, MdEdit, MdAddPhotoAlternate } from "react-icons/md";
...
function EditItem() {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const result = await axios.post(`${serverUrl}/item/editItem/${itemId}`, formData, { withCredentials: true })
    ...
    useEffect(() => {
        const handleGetItemById = async () => {
            try {
                const result = await axios.get(`${serverUrl}/item/getItem/${itemId}`, { withCredentials: true })
            ...
            <div className='max-w-md w-full bg-white shadow-lg rounded-xl p-6 border border-[#DDEEE3]'>
                <div className='flex flex-col items-center mb-4'>
                    <div className='bg-[#DDEEE3] p-3 rounded-full mb-3'>
                        <MdEdit className='text-[#2ECC71] w-12 h-12' />
                    </div>
                    <div className="text-2xl font-bold text-gray-800">
                        Edit Food Item
                    </div>
                    {currentItem && (
                        <p className="text-sm text-gray-600 mt-1">Editing: {currentItem.name}</p>
                    )}
                </div>
                ...
                <form className='space-y-4' onSubmit={handleSubmit}>
                    ...
                    <div>
                        ...
                        {frontendImage && (
                            <div className='mt-3'>
                                <div className='text-xs text-gray-500 mb-1'>Current/New Image:</div>
                                <img 
                                    src={frontendImage} 
                                    alt="Preview" 
                    ...
                    <button 
                        ...
                        disabled={loading}
                    >
                        {loading ? <ClipLoader size={20} color='white' /> : "Update Food Item"}
                    </button>
                </form>
            </div>
        </div>
